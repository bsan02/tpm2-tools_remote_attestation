...
SERVICEPROVIDER_IP="10.53.108.121"
...
request_device_service() {
    ...
    process_device_registration_processing_with_device #this line is from privacy_ca --> device_node so no need to change
    if [ $? == 1 ];then
        LOG_ERROR "AIK received from service provider is not on the device"
        return 1
    fi

    cp rsa_ek.pub Registered_EK_Pool # does not need to be changed
    fdupes --recurse --omitfirst --noprompt --delete --quiet \
    Registered_EK_Pool | grep -q rsa_ek.pub
    retval=$?
    ...
    cp p_d_service_token.txt $device_location # does not need to be changed 
