...
privacy_ca_location=""
device_loaction=""
DEVICENODE_IP="10.53.97.206" # device_node IP in for the external host
# State
event_file_found=0
...
GOLDEN_PCR="413162b3e3a7cfb922f7f9d9301289aa01b2820a0e3b87c99a71095a26a3d54f"
# Service Data
...
registration_token: $REGISTRATION_TOKEN
   ...
   #------------------------
   scp s_p_registration.txt root@$DEVICENODE_IP:$pca_location/.
   #------------------------
   ...
   scp s_d_registration.txt root@$DEVICENODE_IP:$device_location/.
...
echo "$data_to_privacy_ca" > s_p_service.txt
    scp s_p_service.txt root@$DEVICENODE_IP:$pca_location/.
    #cp s_p_service.txt $pca_location/. # cp is unchanged bc its communicating with PCA location
    rm -f s_p_service.txt
    # Send privacy-CA information to device
    echo "privacy_ca_location: $pca_location" > s_d_service.txt
    #cp s_d_service.txt $device_location
    #--------------------------
    scp s_d_service.txt root@$DEVICENODE_IP:$device_location/
    #--------------------------
   ...
   if [ $? == 1 ];then
      ...
      scp d_s_service_aik.pub root@$DEVICENODE_IP:$pca_location/s_p_service_aik.pub # remains unchanged bc its communicating with PCA location
      #cp d_s_service_aik.pub $pca_location/s_p_service_aik.pub # remains unchanged bc its communicating with PCA location
   fi
...
system_software_state_validation() {
 ...
 #--------------------
   scp s_d_pcrlist.txt root@$DEVICENODE_IP:$device_location/.
   #--------------------
...
#    -in service-content.plain -out s_d_service_content.encrypted
    openssl pkeyutl -encrypt -inkey d_s_service_content_key.pub -pubin \
    -in service-content.plain -out s_d_service_content.encrypted -pkeyopt rsa_padding_mode:oaep \
    -pkeyopt rsa_oaep_md:sha256
#   openssl pkeyutl \
#     -encrypt \
#     -pubin \
#     -inkey d_s_service_content_key.pub \
#     -in service-content.plain \
#     -out s_d_service_content.encrypted \
#     -pkeyopt rsa_padding_mode:oaep
    scp s_d_service_content.encrypted root@$DEVICENODE_IP:$device_location/.
    #cp s_d_service_content.encrypted $device_location/. # needs to change bc its communicating with device_location
    rm -f d_s_service_aik.pub
    rm -f d_s_service_content_key.pub
